...
def extract_invoice_fields(text: str) -> dict:
    ...
    # --- fallback 2: last resort — grab a standalone currency-looking number,
    # excluding anything that's actually part of the invoice number, date, or
    # tax value (so we don't accidentally grab e.g. "8822" from "FL-8822") ---
    if amount is None:
        exclude_digits = set()
        if invoice_no:
            exclude_digits.update(re.findall(r"\d+", invoice_no))
        if date_raw:
            exclude_digits.update(re.findall(r"\d+", date_raw))
        if tax is not None:
            exclude_digits.add(str(tax))
            if tax == int(tax):
                exclude_digits.add(str(int(tax)))

        # (?<![A-Za-z0-9/-]) / (?![A-Za-z0-9/-]) stop us from matching a
        # fragment of a code like "FL-8822" or a date like "2026-01-01"
        candidates = re.findall(
            r"(?:Rs\.?|INR|₹|\$|USD|€|EUR|£|GBP)\s*([\d,]+(?:\.\d{1,2})?)"
            r"|(?<![A-Za-z0-9/-])(\d{2,}(?:,\d{2,3})*(?:\.\d{1,2})?)(?![A-Za-z0-9/-])",
            text,
        )
        for a, b in candidates:
            raw_val = a or b
            val = parse_amount(raw_val)
            if val is None or val == tax:
                continue
            digits_only = re.sub(r"[.,]", "", raw_val)
            if digits_only in exclude_digits:
                continue
            amount = val
            break
